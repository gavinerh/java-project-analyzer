...
public class TestingPurposes {
    public static void main(String[] args) throws Exception {
        String val1 = "SELECT AirNonAir, SUM(NVL(Pps_value,0)) AS LONG1 from (SELECT P.ANA_IND AS AirNonAir,\n" +
                "        NVL(SUM(DECODE(AT.trans_cd,\n" +
                "        'TC', AT.PPS_VAL, 'TD', -AT.PPS_VAL, 'SC', AT.PPS_VAL, 'SD', -AT.PPS_VAL)),0) Pps_value FROM AT_TRANS AT INNER\n" +
                "        JOIN PRT P ON P.PRT_CD = DECODE(AT.CD_SHARE_PRT, NULL, AT.PRT_CD, AT.CD_SHARE_PRT ) Where AT.Int_Id =\n" +
                "        #{internalID,jdbcType=NUMERIC} And AT.Prg_Cd = 'KF' AND AT.PPS_BUCKET_FLG = 'Y' And AT.Flt_Awd_Dt Between\n" +
                "        trunc(#{startDt,jdbcType=TIMESTAMP}) and trunc(#{endDt,jdbcType=TIMESTAMP}) And AT.Trans_Cd In ('TC', 'TD',\n" +
                "        'SC', 'SD') And DECODE(AT.CD_SHARE_PRT, NULL, AT.PRT_CD, AT.CD_SHARE_PRT )\n" +
                "        <choose>\n" +
                "            <when test='!isAfter'>\n" +
                "                IN (SELECT REF_REC_CD from REFERENCE_CD where REF_REC_TYPE =885)\n" +
                "            </when>\n" +
                "            <otherwise>\n" +
                "                IN (SELECT REF_REC_CD from REFERENCE_CD where REF_REC_TYPE =886)\n" +
                "            </otherwise>\n" +
                "        </choose>\n" +
                "        GROUP BY P.ANA_IND UNION ALL SELECT\n" +
                "        P.ANA_IND AS AirNonAir, NVL(SUM(DECODE(PCM.trans_cd, 'MC', PCM.CUR_PPS_VAL, 'MD', -PCM.CUR_PPS_VAL)),0)\n" +
                "        Pps_value FROM PRT_CUS_MERGE PCM INNER JOIN PRT P ON P.PRT_CD = PCM.PRT_CD WHERE PCM.INT_ID\n" +
                "        =#{internalID,jdbcType=NUMERIC} And PCM.Prg_Cd = 'KF' And PCM.CUR_PPS_VAL != 0 And PCM.Batch_Dt Between\n" +
                "        trunc(#{startDt,jdbcType=TIMESTAMP}) and trunc(#{endDt,jdbcType=TIMESTAMP}) And PCM.Trans_Cd In ('MC', 'MD') AND\n" +
                "        PCM.PRT_CD\n" +
                "        <choose>\n" +
                "            <when test='!isAfter'>\n" +
                "                IN (SELECT REF_REC_CD from REFERENCE_CD where REF_REC_TYPE =885)\n" +
                ...
                "        </choose>\n" +
                "        GROUP BY P.ANA_IND UNION ALL SELECT\n" +
                "        P.ANA_IND AS AirNonAir, NVL(SUM(DECODE(PT.trans_cd, 'ZC', PT.PPS_BONUS_VALUE_AWDED, 'ZD',\n" +
                "        -PT.PPS_BONUS_VALUE_AWDED)),0) Pps_value FROM PROMO_TRANS PT INNER JOIN PRT P ON P.PRT_CD = PT.PRT_CD WHERE\n" +
                "        PT.INT_ID =#{internalID,jdbcType=NUMERIC} AND PT.PRG_CD = 'KF' And PT.PPS_BONUS_VALUE_AWDED != 0 AND PT.Batch_Dt\n" +
                "        Between trunc(#{startDt,jdbcType=TIMESTAMP}) and trunc(#{endDt,jdbcType=TIMESTAMP}) And PT.Trans_Cd In ('ZC',\n" +
                "        'ZD')\n" +
                "        <choose>\n" +
                "            <when test='!isAfter'>\n" +
                "                IN (SELECT REF_REC_CD from REFERENCE_CD where REF_REC_TYPE =885)\n" +
                ...
                "            </otherwise>\n" +
                "        </choose>\n" +
                "        GROUP BY P.ANA_IND ) group by AirNonAir";
        String old = "SELECT AirNonAir, SUM(NVL(Pps_value,0)) AS LONG1 from (SELECT P.ANA_IND AS AirNonAir,\n" +
                "        NVL(SUM(DECODE(AT.trans_cd, 'TC', AT.PPS_VAL, 'TD', -AT.PPS_VAL, 'SC', AT.PPS_VAL, 'SD', -AT.PPS_VAL)),0)\n" +
                "        Pps_value FROM AT_TRANS AT INNER JOIN PRT P ON P.PRT_CD = decode(AT.CD_SHARE_PRT, null, AT.CD_SHARE_PRT, 'MI',\n" +
                "        AT.CD_SHARE_PRT, AT.PRT_CD ) Where AT.Int_Id = #{internalId,jdbcType=NUMERIC} And AT.Prg_Cd = 'KF' AND\n" +
                "        AT.PPS_BUCKET_FLG = 'Y' And AT.Flt_Awd_Dt Between trunc(#{startDt,jdbcType=TIMESTAMP}) and\n" +
                "        trunc(#{endDt,jdbcType=TIMESTAMP}) And AT.Trans_Cd In ('TC', 'TD', 'SC', 'SD') And decode(AT.CD_SHARE_PRT, 'SQ',\n" +
                "        AT.CD_SHARE_PRT, 'MI', AT.CD_SHARE_PRT, AT.PRT_CD )\n" +
                "        <choose>\n" +
                "            <when test=\"!isAfter\">\n" +
                "                IN ('SQ','MI')\n" +
                "            </when>\n" +
                ...
                "            </otherwise>\n" +
                "        </choose>\n" +
                "        GROUP BY P.ANA_IND UNION ALL SELECT P.ANA_IND AS AirNonAir, NVL(SUM(DECODE(PCM.trans_cd,\n" +
                "        'MC', PCM.CUR_PPS_VAL, 'MD', -PCM.CUR_PPS_VAL)),0) Pps_value FROM PRT_CUS_MERGE PCM INNER JOIN PRT P ON P.PRT_CD\n" +
                "        = PCM.PRT_CD WHERE PCM.INT_ID =#{internalId,jdbcType=NUMERIC} And PCM.Prg_Cd = 'KF' And PCM.CUR_PPS_VAL != 0 And\n" +
                "        PCM.Batch_Dt Between trunc(#{startDt,jdbcType=TIMESTAMP}) and trunc(#{endDt,jdbcType=TIMESTAMP}) And\n" +
                "        PCM.Trans_Cd In ('MC', 'MD') AND PCM.PRT_CD\n" +
                "        <choose>\n" +
                "            <when test=\"!isAfter\">\n" +
                "                IN ('SQ','MI')\n" +
                "            </when>\n" +
                "            <otherwise>\n" +
                "                IN (SELECT REF_REC_CD from REFERENCE_CD where\n" +
                "                REF_REC_TYPE =886)\n" +
                "            </otherwise>\n" +
                "        </choose>\n" +
                "        GROUP BY P.ANA_IND UNION ALL SELECT P.ANA_IND AS AirNonAir, NVL(SUM(DECODE(PT.trans_cd, 'ZC',\n" +
                "        PT.PPS_BONUS_VALUE_AWDED, 'ZD', -PT.PPS_BONUS_VALUE_AWDED)),0) Pps_value FROM PROMO_TRANS PT INNER JOIN PRT P ON\n" +
                "        P.PRT_CD = PT.PRT_CD WHERE PT.INT_ID =#{internalId,jdbcType=NUMERIC} AND PT.PRG_CD = 'KF' And\n" +
                "        PT.PPS_BONUS_VALUE_AWDED != 0 AND PT.Batch_Dt Between trunc(#{startDt,jdbcType=TIMESTAMP}) and\n" +
                "        trunc(#{endDt,jdbcType=TIMESTAMP}) And PT.Trans_Cd In ('ZC', 'ZD')\n" +
                "        <if test=\"isAfter\">\n" +
                "            AND PT.PRT_CD IN (SELECT REF_REC_CD from REFERENCE_CD where REF_REC_TYPE =886)\n" +
                "        </if>\n" +
                "        GROUP BY P.ANA_IND ) group by AirNonAir";
...
//        printUniqueSet(uniqueNumbers);
    ...
    private static void compare(String val1, String old) {
        StringBuilder stringOld = new StringBuilder();
        StringBuilder stringNew = new StringBuilder();
        val1 = val1.trim().toLowerCase();
        old = old.trim().toLowerCase();
        int newCount = 0;
        int oldCount = 0;
        while(newCount < val1.length() || oldCount < old.length()) {
            stringOld.append(old.charAt(oldCount));
            stringNew.append(val1.charAt(newCount));
            if(val1.charAt(newCount) == '\n' || val1.charAt(newCount) == '\t' || val1.charAt(newCount) == ' ') {
                newCount++;
                continue;
            }
            if(old.charAt(oldCount) == '\n' || old.charAt(oldCount) == '\t' || old.charAt(oldCount) == ' ') {
                oldCount++;
                continue;
            }
            if(val1.charAt(newCount) != old.charAt(oldCount)) {
                System.out.println("Difference found at new index: " + newCount + " and old index: " + oldCount);
                System.out.println("New string so far: " + stringNew.toString() + " <<DIFF>> " + val1.substring(newCount));
                System.out.println("Old string so far: " + stringOld.toString() + " <<DIFF>> " + old.substring(oldCount));
                return;
            }
            newCount++;
            oldCount++;
